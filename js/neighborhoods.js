...
function setupMapEvents() {
  ...
  if (hoodMap.getLayer('locality-fill')) {
    hoodMap.on('click', 'locality-fill', function(e) {
      ...
      if (bounds) {
        hoodMap.fitBounds(bounds, { padding: 60, maxZoom: 15, bearing: 29 });
      }
    });
...
function zoomToFeature(feature) {
  ...
  hoodMap.fitBounds(bbox, { padding: 60, maxZoom: 15, bearing: 29 });
}

function zoomToPoint(lat, lng, zoomLevel) {
  if (!hoodMap) return;
  hoodMap.flyTo({ center: [lng, lat], zoom: zoomLevel || 15, bearing: 29 });
}
